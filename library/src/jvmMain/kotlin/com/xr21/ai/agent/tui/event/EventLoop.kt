...
class EventLoop(
     ...
     * 优先匹配快捷键映射，再处理普通字符输入
     * Up/Down/Enter/Esc 根据当前焦点面板和弹框状态做上下文分发
     */
    private fun resolveAction(keyEvent: KeyEvent): Action? {
        // 如果会话列表弹框可见，拦截按键
        if (appState.isSessionListPopupVisible) {
            return when (keyEvent) {
                KeyEvent.Up -> Action.SELECT_UP
                KeyEvent.Down -> Action.SELECT_DOWN
                KeyEvent.Enter -> Action.SELECT_CONFIRM
                KeyEvent.Escape -> Action.NOOP
                else -> null
            }
        }
    ...
    private fun handleInputChar(keyEvent: KeyEvent): Action? {
        return when (keyEvent) {
            ...
            KeyEvent.Enter -> {
                // 弹框可见时由 resolveAction 拦截，不会走到这里
                // Enter 在 Input 焦点时发送消息
                if (appState.focusPanel == PanelType.INPUT && appState.inputBuffer.isNotBlank()) {
                    Action.SEND_MESSAGE
                } else {
                    null
                }
