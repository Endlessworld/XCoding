...
class AppState {
    ...
    /**
     * 当前焦点面板（三栏布局：LEFT=侧栏, CENTER=消息区, INPUT=输入框）
     * RIGHT 面板已合并到信息面板中，不再单独作为焦点
     */
    var focusPanel: PanelType = PanelType.CENTER
        private set

    /** 会话列表弹框是否显示 */
    var isSessionListPopupVisible: Boolean = false

    /** 侧边栏选中索引（独立于 currentSessionIndex，用于预览选择） */
    var sidebarSelectedIndex: Int = 0

    /** 切换焦点到下一个面板（CENTER → INPUT → CENTER 循环） */
    fun focusNext() {
        focusPanel = when (focusPanel) {
            PanelType.LEFT -> PanelType.CENTER
            PanelType.CENTER -> PanelType.INPUT
            PanelType.INPUT -> PanelType.CENTER
        }
        onFocusChanged()
    }

    /** 切换焦点到上一个面板（CENTER → INPUT → CENTER 循环） */
    fun focusPrevious() {
        focusPanel = when (focusPanel) {
            PanelType.LEFT -> PanelType.CENTER
            PanelType.CENTER -> PanelType.INPUT
    ...
    /** 关闭会话列表弹框 */
    fun closeSessionListPopup() {
        isSessionListPopupVisible = false
    }

    /** 从弹框中确认选择会话 */
    fun popupConfirmSelection() {
        if (sidebarSelectedIndex in sessions.indices) {
