...
fun main() = runBlocking {
    ...
    session.close()
    try { acpClient.logout() } catch (_: Exception) {}
    httpClient.close()
...
private suspend fun handleCommand(
    ...
    return when (parts[0]) {
        ...
        "/mode" -> { handleModeCommand(session, parts); true }
        "/model" -> { handleModelCommand(session, parts); true }
        else -> {
                ...
                is Event.SessionUpdateEvent -> {
                    when (val update = event.update) {
                        ...
                        is SessionUpdate.ToolCall -> {
                            println()
                            println("  ${Color.YELLOW}tool: ${update.title}${Color.RESET} (${update.toolCallId.value}) ${Color.DIM}PENDING${Color.RESET}")
                        }
                        is SessionUpdate.ToolCallUpdate -> {
                            val status = update.status
                            if (status != null) {
                                println("  ${Color.YELLOW}tool: ${update.title}${Color.RESET} ${status}")
                            }
                        }
