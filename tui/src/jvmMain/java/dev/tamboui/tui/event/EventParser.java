 ...
 *
 * Modified by XR21 Team to support Alt+Enter key combination.
 */
...
public final class EventParser {
    ...
    private static Event parseControlChar(int c, Bindings bindings) {
        switch (c) {
            ...
            case 0:
                // Ctrl+Space (NUL) — mapped to Ctrl+Space for newline insertion
                return KeyEvent.ofChar(' ', KeyModifiers.CTRL, bindings);
            case 28:
                // Ctrl+\ (FS) — mapped to Ctrl+\ for newline insertion
                return KeyEvent.ofChar('\\', KeyModifiers.CTRL, bindings);
            default:
                if (c >= 1 && c <= 26) {
    ...
    private static Event parseEscapeSequence(Backend backend, Bindings bindings) throws IOException {
        ...
        // Alt+key (printable characters)
        backend.read(PEEK_TIMEOUT); // consume the character
        if (next >= 32 && next != 127) {
            return KeyEvent.ofChar(next, KeyModifiers.ALT, bindings);
        }

        // Alt+Enter (ESC + \n or ESC + \r) — treat as Enter with Alt modifier
        if (next == 10 || next == 13) {
            return KeyEvent.ofKey(KeyCode.ENTER, KeyModifiers.ALT, bindings);
        }
    ...
    private static Event readPasteContent(Backend backend, Bindings bindings) throws IOException {
        ...
        while (true) {
            ...
            if (backend.peek(PEEK_TIMEOUT) != '[') {
                sb.appendCodePoint(ESC);
                continue;
            }
            backend.read(PEEK_TIMEOUT); // consume '['
            if (backend.peek(PEEK_TIMEOUT) != '2') {
                sb.appendCodePoint(ESC);
                sb.appendCodePoint('[');
                continue;
            }
            backend.read(PEEK_TIMEOUT);
            if (backend.peek(PEEK_TIMEOUT) != '0') {
                sb.appendCodePoint(ESC);
                sb.appendCodePoint('[');
            ...
            backend.read(PEEK_TIMEOUT);
            if (backend.peek(PEEK_TIMEOUT) != '1') {
                sb.appendCodePoint(ESC);
                sb.appendCodePoint('[');
                sb.appendCodePoint('2');
